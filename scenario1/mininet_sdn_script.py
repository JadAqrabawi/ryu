...
def scenario1_topology():
    ...
    # Add direct routes between APs
    ap1.cmd('ip route add 10.0.2.0/24 via 0.0.0.0 dev ap1-wlan1')
    ap1.cmd('ip route add 10.0.3.0/24 via 0.0.0.0 dev ap1-wlan1')
    ap2.cmd('ip route add 10.0.1.0/24 via 0.0.0.0 dev ap2-wlan1')
    ap2.cmd('ip route add 10.0.3.0/24 via 0.0.0.0 dev ap2-wlan1')
    ap3.cmd('ip route add 10.0.1.0/24 via 0.0.0.0 dev ap3-wlan1')
    ap3.cmd('ip route add 10.0.2.0/24 via 0.0.0.0 dev ap3-wlan1')
    
    # Set default gateways for stations
    stations = [sta1, sta2, sta3, sta4, sta5, sta6, sta7, sta8, sta9, sta10]
    for sta in stations:
    ...
    # Create target list and start attack
    ...
    for sta in stations:
        subnet = sta.IP().split('.')[2]
        gateway = '10.0.{}.254'.format(subnet)
